...
export default {
  ...
  methods: {
    ...
    axisLabelFormatter(val) {
      return `${utils.number.format(val, { ...this.vConfig, point: 0 })}${
        this.vConfig.unitLabel
      }`
    },
    wrapRadar() {
      if (!this.radar) return
      this.radar.axisName = this.radar.axisName || {
        formatter: this.axisNameFormatter,
      }
      this.radar.axisName.formatter =
        this.radar.axisName.formatter || this.axisNameFormatter
      this.radar.axisLabel = this.radar.axisLabel || {
        formatter: this.axisLabelFormatter,
      }
      this.radar.axisLabel.formatter =
        this.radar.axisLabel.formatter || this.axisLabelFormatter
    },
    wrapSeries() {
      if (!this.series) {
        return
